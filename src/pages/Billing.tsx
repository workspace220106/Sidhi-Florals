...
import { ShoppingBag, Trash2, Plus, Minus, UserCircle, Sparkles, Pencil, ArrowRight, Zap, FileText } from "lucide-react";
...
import { QuickBillGenerator } from "@/components/QuickBillGenerator";
import { RecentBillsTab } from "@/components/RecentBillsTab";
...
type BillingSectionTab = "pos" | "quick-bill" | "recent-bills";

export default function Billing() {
  const [sectionTab, setSectionTab] = useState<BillingSectionTab>("pos");
  ...
  const checkout = () => {
    ...
    createSale.mutate({ customer_id: customerId ? Number(customerId) : null, items, total: totals.total, amount_paid: totals.amountPaid }, {
      onSuccess: (sale) => { clear(); setReceipt(sale); setMobileTab("products"); toast.success(`Bill #${sale.id} generated`); if (totalRef.current) burstPetals(totalRef.current, 14); },
  ...
  return (
    <div className="w-full min-w-0 max-w-full lg:h-[calc(100vh-5.5rem)] flex flex-col">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
        <PageHeader 
          title="Billing & POS" 
          subtitle="Generate bills, print receipts, and share invoices directly via WhatsApp." 
        />
        <Tabs
          id="billing-nav"
          value={sectionTab}
          onChange={(v) => setSectionTab(v as BillingSectionTab)}
          items={[
        ...
        <>
          <div className="lg:hidden flex p-1 bg-secondary-soft rounded-xl mb-4 border border-border">
            {(["products", "cart"] as const).map((t) => (
              <button key={t} onClick={() => setMobileTab(t)} className={cn("flex-1 py-2.5 rounded-lg font-semibold text-sm cursor-pointer transition-all duration-200", mobileTab === t ? "bg-surface text-primary shadow-card font-bold" : "text-muted")}>
          ...
          <div className="flex-1 min-h-0 w-full min-w-0 flex flex-col lg:flex-row gap-4 xl:gap-6">
            <section className={cn("flex-1 min-w-0 min-h-0 flex-col gap-3.5", mobileTab === "products" ? "flex" : "hidden lg:flex")}>
              ...
              <div className="p-3.5 sm:p-4 border-b border-border bg-secondary-soft/50">
                ...
                <div className="mt-2.5 flex items-center gap-2">
                  ...
                  <Select value={customerId} onChange={(e) => setCustomerId(e.target.value)} className="py-1.5 text-xs sm:text-sm bg-surface">
                    ...
                    {customers?.map((c) => <option key={c.id} value={c.id}>{c.name}{c.phone ? ` (${c.phone})` : ""}{c.business ? ` · ${c.business}` : ""}</option>)}
                  </Select>
                </div>
              </div>
