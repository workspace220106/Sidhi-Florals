    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-full min-h-0">
      ...
      <div className="flex flex-col min-h-0">
              ...
              return (
                <m.div key={c.productId} layout initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }} transition={{ duration: 0.18 }}
                  ...
                  <div className="flex items-center gap-1 bg-surface rounded-lg border border-border p-0.5">
                    <button type="button" aria-label={`Decrease ${c.name}`} onClick={() => setQty(p, -1)} className="p-1 rounded hover:bg-primary-soft cursor-pointer"><Minus className="w-3 h-3" /></button>
                    ...
                    <button type="button" aria-label={`Increase ${c.name}`} onClick={() => setQty(p, 1)} className="p-1 rounded hover:bg-primary-soft cursor-pointer"><Plus className="w-3 h-3" /></button>
                  </div>
                  <button type="button" aria-label={`Remove ${c.name}`} onClick={() => setQty(p, -c.quantity)} className="p-1.5 text-muted hover:text-danger cursor-pointer"><Trash2 className="w-4 h-4" /></button>
                </m.div>
              );
            })}
          </AnimatePresence>
        ...
        <div className="mt-3 pt-3 border-t border-border space-y-3">
          ...
          <Button type="button" className="w-full" onClick={submit} disabled={!canSubmit} loading={submitting}>
